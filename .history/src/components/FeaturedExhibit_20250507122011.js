...
const FeaturedExhibit = () => {
  ...
  useEffect(() => {
    ...
    panelsRef.current.forEach((panel, index) => {
      gsap.set(panel, { 
        width: panelWidth,
        x: index === 0 ? 0 : panelWidth // Only first panel visible initially
      });
    });

    // Create master timeline for horizontal scroll
    ...
    // Add panel-specific animations with delays
    panelsRef.current.forEach((panel, index) => {
      if (index > 0) {
        // Entry animation for subsequent panels
        tl.to(panel, {
          x: 0,
          duration: 0.6,
          ease: "power2.out",
          delay: 0.2 // Slight delay before entering
        }, index * 0.8); // Staggered start times
        
        // Exit animation for previous panel
        if (index > 0) {
          tl.to(panelsRef.current[index - 1], {
            x: -panelWidth * 0.3, // Move slightly left
            opacity: 0.7,
            duration: 0.5,
            ease: "power2.in"
          }, index * 0.8 - 0.3); // Start slightly before new panel enters
        }
      }
    });

    // Enhanced snap points with momentum
    ScrollTrigger.create({
      ...
      snap: {
        snapTo: (value, self) => {
          ...
          const panelProgress = value * (featuredExperiences.length - 1);
          const panelIndex = Math.round(panelProgress);
          
          // Add momentum-based snapping
          const velocity = self.getVelocity() / 1000;
          if (Math.abs(velocity) > 0.5) {
            return (panelIndex + (velocity > 0 ? 1 : -1)) / (featuredExperiences.length - 1);
          }
          return panelIndex / (featuredExperiences.length - 1);
        },
        duration: { min: 0.4, max: 0.8 }, // Longer, smoother snap
        ...
        <div className="exhibit-scroll-wrapper" ref={containerRef}>
          {featuredExperiences.map((experience, index) => (
            <div
              ...
              style={{
                backgroundColor: [
                  '#f0f8ff', // Light blue for first panel
                  '#fff0f5', // Lavender blush for second
                  '#f0fff0', // Honeydew for third
                  '#fff8dc'  // Cornsilk for fourth
                ][index % 4]
              }}
            >
