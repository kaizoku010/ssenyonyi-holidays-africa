...
const FeaturedExhibit = () => {
  ...
  useEffect(() => {
    ...
    const initAnimation = () => {
      ...
      console.log("Initializing GSAP animation with", panelsRef.current.length, "panels");
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          start: "top 25%", // Start when the section is 25% from the top
          end: `+=${(panelsRef.current.length - 1) * 100}%`, // Each panel gets 100% of viewport height
      ...
      // Create the panel transitions
      // Each panel gets an equal portion of the scroll
      const panelCount = panelsRef.current.length;
      const increment = 1 / (panelCount - 1);
