  ...
  useEffect(() => {
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        end: () => `${window.innerHeight * featuredExperiences.length}`,
    ...
    // Enhanced snapping with precise panel positions
    ScrollTrigger.create({
      ...
      snap: {
        snapTo: (value) => {
          ...
          const increment = 1 / (featuredExperiences.length - 1);
          for (let i = 0; i < featuredExperiences.length; i++) {
            panelPositions.push(i * increment);
          }
