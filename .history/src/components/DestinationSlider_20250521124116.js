...
import kitandra8 from '../media/ants.png';
...
const DestinationSlider = ({
  ...
  const nextSlide = () => {
    if (!isTransitioning) {
      ...
      // Start fading in the next slide before the current one completely fades out
      // This creates an overlap in the transition
      setTimeout(() => {
        setCurrentSlide((prev) => (prev + 1) % sliderDestinations.length);
      }, 300); // Start changing slide when current cards are at 50% opacity
  ...
  const prevSlide = () => {
    if (!isTransitioning) {
      ...
      // Start fading in the previous slide before the current one completely fades out
      setTimeout(() => {
