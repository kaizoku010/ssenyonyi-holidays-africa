...
const featuredExperiences = [
  {
    ...
    features: [
      '7-day immersive safari experience',zzzzzzzzzz
    ...
    const initAnimation = () => {
      ...
      panelsRef.current.forEach((panel, i) => {
        if (i === 0) {
          ...
          gsap.set(panel, { xPercent: 0, opacity: 1, visibility: "visible" });
        } else {
          // Other panels are hidden off-screen but with full opacity
          gsap.set(panel, { xPercent: 100, opacity: 1, visibility: "hidden" });
        }
      });
      ...
      for (let i = 0; i < panelCount - 1; i++) {
        ...
        // Bring next panel in from the right with full opacity
        tl.fromTo(
          panelsRef.current[i + 1],
          { xPercent: 100, opacity: 1, visibility: "hidden" },
          {
            xPercent: 0,
        ...
        // Add a specific point to ensure the panel stays fully visible
        if (i < panelCount - 2) {
          tl.to(panelsRef.current[i + 1], {
            xPercent: 0,
            opacity: 1, // Force full opacity during plateau
