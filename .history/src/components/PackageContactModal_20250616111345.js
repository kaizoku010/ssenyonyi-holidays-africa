...
const PackageContactModal = ({ isOpen, onClose, packageData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Debug: Log the package data and form data
      ...
      const submissionData = {
        ...
        package_id: packageData.id || 1, // Fallback to 1 if no ID
        customer_name: formData.name,
        customer_email: formData.email,
        customer_phone: formData.phone || '',
        message: formData.message || '',
        preferred_date: formData.preferredDate || null,
        number_of_people: parseInt(formData.numberOfPeople) || 2,
        package_price: packageData.price || 'TBD',
        package_duration: packageData.duration || 'TBD',
      ...
      try {
        ...
        if (emailResult.success) {
          ...
          alert('✅ EMAILS SENT! Check your inbox!');
        } else {
          console.error('❌ Email sending failed:', emailResult.error);
          alert('❌ EMAIL FAILED: ' + emailResult.error);
        }
      } catch (emailError) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h2 className='form-title' >Inquire About This Package</h2>
