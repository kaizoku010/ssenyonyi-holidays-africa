...
const GalleryPage = () => {
  ...
  // Create a more dynamic mosaic layout
  const createMosaicLayout = () => {
    // Add size class to each image
    const imagesWithSizeClass = filteredImages.map(image => ({
      ...image,
      sizeClass: getSizeClass(image)
    }));

    // Distribute images across columns more intelligently based on size
    const columnArrays = Array.from({ length: columns }, () => []);
    const columnHeights = Array(columns).fill(0);

    imagesWithSizeClass.forEach(image => {
      ...
      // Update column height (approximation based on aspect ratio)
      let heightAddition = 1;
      if (image.sizeClass === 'tall') heightAddition = 1.5;
      if (image.sizeClass === 'wide') heightAddition = 0.7;

      columnHeights[shortestColumnIndex] += heightAddition;
    });

    return columnArrays;
  };

  const mosaicLayout = createMosaicLayout();
    ...
    <div className="gallery-page">
      ...
      <main className="gallery-content">
        ...
        <div className="gallery-mosaic">
          {mosaicLayout.map((column, columnIndex) => (
            <div key={columnIndex} className="mosaic-column">
              {column.map(image => (
                <div
                  key={image.id}
